/**
 * The "Klemmen" tab: the derived terminal list per FI group, the Stückliste,
 * the bulk switches, and the rail tile's marker. The derivation itself is
 * pinned in `schaltplanTerminals.test.ts`; this proves the tab shows it,
 * that the two bulk buttons fire the right flag and vanish read-only, and
 * that a part whose width is unconfirmed is said out loud.
 */
    ...
    rows: [
      {
        ...
        devices: [
          ...
          makeDevice("mcb", { id: "a", designation: "F1.1", circuit: "1", label: "Licht Flur", terminal_block: true }),
...
describe("TerminalList", () => {
  it("shows one card per FI group with the derived sequence", () => {
    renderList();
    const first = screen.getByRole("region", { name: "FI F1 · Reihe 1" });
    ...
    expect(screen.getByRole("region", { name: "FI F2 · Reihe 1" })).toBeInTheDocument();
  });
  ...
  it("lists both print modes per terminal, never one filled in from the other", () => {
    const document = terminalBoard();
    // No Stromkreis-Nr. yet on F1.1: the default print leaves that marker blank.
    ...
    const rows = within(table)
      ...
      .map((row) => within(row).getAllByRole("cell").map((cell) => cell.textContent));
    expect(rows).toEqual([
      ["1", "WAGO 2016-7714", "F1", "F1", "FI F1 · 30 mA / Typ A"],
      ["2", "WAGO 2003-7641", "—", "F1.1", "F1.1 · Licht Flur"],
      ["3", "WAGO 2009-305", "—", "—", "—"],
    ]);
    // The phone card says the same two things.
    const cards = Array.from(first.querySelectorAll(".sp-terminal-row b")).map((node) => node.textContent);
    expect(cards[1]).toBe("Nr. — · BMK F1.1 · WAGO 2003-7641");
  });
  ...
  it("flags a group without an FI", () => {
    ...
    renderList({ document });
    expect(screen.getByRole("region", { name: "HS Q1 · Reihe 1" })).toBeInTheDocument();
