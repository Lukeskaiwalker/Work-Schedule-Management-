...
What a *paired* station then does — authenticate, heartbeat, be listed and
revoked — lives in ``workflow_station.py``; the token primitives the two
share (``hash_token``, the prefix, the entropy) are defined there and imported
here, in that one direction only.
"""
...
from app.routers.workflow_station import STATION_TOKEN_PREFIX, TOKEN_RANDOM_BYTES, hash_token
...
from app.services.station_view import UNCLAIMED_HASH_PREFIX, is_station_claimed, station_out
...
# ``expires_in_days: null`` to opt out. (The schema default on
# ``StationPairApproveRequest.expires_in_days`` is what enforces it.)
...
# collects it, ``token_hash`` holds a sentinel no sha256 digest can ever equal
# (``services/station_view.UNCLAIMED_HASH_PREFIX``). That keeps the "never
# store a plaintext token" rule absolute without deferring the row. What a
# station looks like on the wire — status, hardware, who paired it — is built
# in ``station_view`` too, so this router and the admin operations describe a
# station identically.


def _unclaimed_hash() -> str:
    return f"{UNCLAIMED_HASH_PREFIX}{secrets.token_hex(16)}"
...
def pair_start(
    ...
    device_token = f"{DEVICE_TOKEN_PREFIX}{secrets.token_urlsafe(TOKEN_RANDOM_BYTES)}"
...
def pair_approve(
    ...
    return StationPairApproveOut(user_code=pairing.user_code, station=station_out(db, station))
...
def pair_poll(
    ...
    pairing = db.scalars(
        select(StationPairing).where(StationPairing.device_token_hash == hash_token(token))
    ).first()
    if pairing is None:
    ...
    if expired:
        ...
        if pairing.status == PAIRING_APPROVED and pairing.station_id is not None:
            ...
            if station is not None and station.revoked_at is None and not is_station_claimed(station):
...
def _issue_station_token(
    ...
    if is_station_claimed(station):  # pragma: no cover — status guard above already covers it
        return StationPairPollOut(
            status="claimed",
            poll_interval=POLL_MIN_INTERVAL_SECONDS,
            message="The station token was already issued and is never reissued.",
    ...
    raw_token = f"{STATION_TOKEN_PREFIX}{secrets.token_urlsafe(TOKEN_RANDOM_BYTES)}"
    station.token_hash = hash_token(raw_token)
    ...
    return StationPairPollOut(
        status="approved",
        token=raw_token,
        station=station_out(db, station),
        poll_interval=POLL_MIN_INTERVAL_SECONDS,
